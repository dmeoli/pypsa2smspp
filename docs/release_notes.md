...
* Translate the reactance of a `Line` into the susceptance of the network of SMS++, i.e., Kirchhoff's voltage law, which the conversion wrote as zero, turning the AC network into a transport model. The conversion also writes, next to the instance, the BlockConfig that asks for the KIRCHHOFF formulation, and gives it to the solver: the one it takes by default disagrees with it on a network of AC lines and HVDC links up to UCBlock `2b69e107`, where the two now agree
