...
* Translate the reactance of a `Line` into the susceptance of the network of SMS++, i.e., impose Kirchhoff's voltage law as PyPSA does, with `Transformation(kirchhoff_voltage_law=True)`; the conversion then also writes the BlockConfig that asks for the KIRCHHOFF formulation and gives it to the solver. It is off by default, an AC network being a transport one as before, because a released SMS++ has neither that formulation nor the fixes the other one needs (UCBlock `2b69e107`)
